...
if (isset($_POST['edit'])) {
    ...
    $req = $access->prepare("INSERT INTO `produits`(`image`, `nom`, `prix`, `description`) VALUES (:img,:nm,:pr,:desc)");
...
<body>
    ...
    <section class="products" id="products" style="margin-top: 10%;">
        <h1 class="heading">Ajouter un nouveau <span>Produit</span></h1>
        <?php
        if (isset($success)) {
            if ($success) {
                echo '<div class="alert alert-success" role="alert">
                    Produit a était ajouter avec succés.
                  </div>';
            } else {
                echo '<div class="alert alert-danger" role="alert">
                    Erreur !! réssayer plus tard.
                  </div>';
            }
        }
        ?>
        <div class="box-container justify-content-center">
            <form method="POST" enctype="multipart/form-data">
                ...
                <div>
                    <button type="submit" class="btn" name="edit">Sauvegardé</button>
                </div>
            </form>
        </div>
